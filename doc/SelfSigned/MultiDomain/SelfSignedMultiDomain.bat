...
openssl req -newkey rsa:2048 -noenc -keyout openssl.asianaidt.com.key -keyform PEM -x509 -sha256 -out openssl.asianaidt.com.crt -outform PEM -days 365 -config openssl.cnf -section req -extensions v3_req -verbose
...
keytool -genkeypair -alias openssl.asianaidt.com -keyalg RSA -keysize 2048 -storetype PKCS12 -keystore keystore.p12 -validity 365 -keypass "openssledu" -storepass "openssledu" -dname "CN=openssl.asianaidt.com, C=KR" -ext "SAN=DNS:openssl.asianaidt.com,DNS:tomcat.asianaidt.com,DNS:mail.asianaidt.com" -ext "KeyUsage=nonRepudiation,digitalSignature,keyEncipherment" -ext "ExtendedKeyUsage=serverAuth"
